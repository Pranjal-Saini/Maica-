...
async def test_records_list_page_links_to_report(client: AsyncClient) -> None:
    ...
    response = await client.get(
        f"/analyses/{analysis_id}/records", headers={"X-Tenant-Id": TENANT_ID}
    )

    assert response.status_code == 200
    assert "text/html" in response.headers["content-type"]
    assert f"/analyses/{analysis_id}/records/1001/report" in response.text
    assert f"/analyses/{analysis_id}/records/1003/report" in response.text


async def test_records_list_page_with_no_records(client: AsyncClient) -> None:
    response = await client.get(
        f"/analyses/{uuid.uuid4()}/records", headers={"X-Tenant-Id": TENANT_ID}
    )
...
async def test_report_page_renders_factors_gaps_and_next_step(client: AsyncClient) -> None:
    ...
    response = await client.get(
        f"/analyses/{analysis_id}/records/1001/report", headers={"X-Tenant-Id": TENANT_ID}
    )
...
async def test_report_page_for_record_with_no_correlations(client: AsyncClient) -> None:
    ...
    response = await client.get(
        f"/analyses/{analysis_id}/records/5001/report", headers={"X-Tenant-Id": TENANT_ID}
    )
